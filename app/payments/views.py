...
def payment_view(request, order_id=None):
    ...
    if request.method == "GET":
        context = {
            "order":order,
            "total_price":int(order.calculate_total_price()),
        }
        ...
        try:
            ...
            amount_cents = int(convert_usd_to_egp(request, order.calculate_total_price())) * 100
            currency = "EGP"
            print("------------------")
            ...
            # order_items = [{"order_id":order_id}] # Error: Expecting value: line 1 column 1 (char 0)
            order_items = []
